...
export IJK_CC=gcc
export IJK_IS_NOT_SUPPORT_ARM=false
export IJK_NDK_REL=$(grep -o '^r[0-9]*.*' $ANDROID_NDK/RELEASE.TXT 2>/dev/null | sed 's/[[:space:]]*//g' | cut -b2-)
case "$IJK_NDK_REL" in
10e*)
    # we don't use 4.4.3 because it doesn't handle threads correctly.
    if test -d ${ANDROID_NDK}/toolchains/arm-linux-androideabi-4.9; then # if gcc 4.8 is present, it's there for all the archs (x86, mips, arm)
        ...
        echo "You need the NDKr10e or later"cd
        exit 1
    fi
    ;;
*)
